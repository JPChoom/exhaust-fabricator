...
Blender 4.2 or newer. Install the release zip as a Blender extension (Edit → Preferences → Get Extensions → Install from Disk, or drag-and-drop onto the Blender window).

## Status

The core Route/Collector/junction/reducer modeling is stable. The assisted-routing / keep-out / guide-spline Header workflow (see the version history below) is still experimental and under active development.
...
New solver capability, prompted by a real fabrication-practice question: is it common to mix centerline radii on the same primary? Yes — a tighter CLR (often 1.0-1.25x tube OD) right off the cylinder head flange to clear a steering shaft, frame rail, or motor mount, transitioning to the standard/sweeping CLR (typically 1.5x+) for the rest of the run, is standard aftermarket and OEM practice. The assisted solver previously used exactly one CLR for the whole generated candidate; it now supports this pattern directly.

New Header settings: **Tighter CLR Near Flange** (`solver_use_tight_start_clr`, off by default) and **Start CLR** (`solver_start_clr`). When enabled, the solver's first bend (the one nearest wherever the search starts) uses Start CLR instead of the standard Routing CLR; the rest of the primary's bends are unaffected.

Implementation: `_candidate()` in `routing_solver.py` now takes an optional `start_radius`, applied only to the first corner's fillet trim and arc-length contribution -- the other two corners keep using `radius` exactly as before. `candidate_segments()`, `candidate_centerline_points()`, and `candidate_capsule_chain()` (previously reading one `candidate['radius']` for every bend) now read a `candidate['radii']` per-bend tuple instead. `start_radius` defaults to `None` everywhere, which reproduces the prior single-radius behavior exactly (verified, not assumed) -- so this is fully backward compatible for every existing caller and every saved file, with no new search dimension: the start radius is a fixed input the user sets to a known physical constraint, not something the solver searches or refines, so it adds no search cost. It is deliberately NOT applied on the "Complete to Collector" path (v0.14.17): that feature's whole premise is that the user has already hand-built the tight, packaging-constrained portion themselves, so applying Start CLR there would tighten the solver's own first bend a second time, in the wrong place (no longer anywhere near the flange).

Verified with a new dedicated test (`test_mixed_clr.py`, not shipped): `start_radius=None` and `start_radius=<same value as radius>` reproduce the exact prior `total_length`/`lengths` (bit-for-bit where floating point allows); a genuinely different start radius changes only the first bend's trim/arc-length contribution and is reflected correctly in `candidate_segments()`'s per-bend radius assignment; the solved centerline still lands exactly on the target position/tangent regardless of which radius the first bend uses (the whole point of the fillet-trim math is that endpoint/tangent are guaranteed by construction); and a full Blender-headless pipeline test confirms Generate Active Primary with Tighter CLR Near Flange enabled produces a route whose first bend is the tighter Start CLR and remaining bends are the standard Routing CLR, while disabling the toggle reproduces uniform-CLR output exactly as before. The existing standalone correctness suites, UI test suite, and the full 4-run smoke test were all re-run and remain unaffected.

## v0.14.20: Primary Count and Port Spacing apply automatically

Small UX fix, requested directly: changing **Primary Count** or **Port Spacing** on a Header used to require a separate button click ("Apply Count" / "Arrange Primary Starts") to actually take effect. Both settings now apply immediately as soon as you change them -- primaries are added/removed and every primary is re-spaced the moment you type or drag a new value, with no extra click. The two buttons have been removed from the Setup panel since there's nothing left for them to do that isn't already automatic.

Implementation: the logic each button ran was factored out of the operators into two shared functions in `operators.py` (`_apply_header_primary_count`, already-existing `_arrange_header_starts`), and `Primary Count`/`Port Spacing` (`properties.py`) each gained an `update` callback that calls them directly via a lazy import (the same pattern already used for the OD/wall-thickness and flange-dimension properties elsewhere on the Header). The two operators (`exhaust.header_apply_count`, `exhaust.header_arrange_starts`) are still registered and fully functional if anything ever needs to re-trigger them directly -- only their buttons were removed, not the operators themselves.

**A real bug this caught before shipping:** `EXHAUST_OT_AddHeader` (the "Add Header" operator) used to set `hs.desired_primary_count` and then *separately* create that many primaries in its own loop -- harmless when the property had no update callback, but with one now wired up, setting the property created the primaries once via the callback and then the operator's own loop created them again, doubling the count (4 requested -> 8 created). Caught immediately by a new dedicated test asserting the primary count after `Add Header`, not discovered after the fact. Fixed by removing the now-redundant manual creation loop entirely -- `Add Header` just sets `desired_primary_count` once and lets the update callback do the rest.

Verified with a new dedicated test (`test_auto_apply_arrange.py`, not shipped): raising/lowering Primary Count adds/removes primaries and re-arranges them with no operator call; changing Port Spacing re-arranges with no operator call; the two buttons are confirmed absent from the Setup panel's actual draw output; and both underlying operators are confirmed still directly callable (registered, not deleted) and still produce correct results on their own. The existing standalone correctness suites, full UI test suite (including the mixed-CLR and Complete-to-Collector suites), and the full 4-run Blender-headless smoke test were all re-run and remain unaffected -- the 4-run smoke test in particular confirms `Add Header` still produces exactly the requested primary count end-to-end.

## v0.14.21: Generate All Primaries always searches from a blank slate

Requested directly: "when clicking Generate All Primaries reset the current layout so it can start from zero." Previously, every phase/order attempt inside Generate All started from whatever was already on each primary before the button was clicked -- leftover geometry from a prior Generate, a hand-built "Complete to Collector" prefix, or manually-edited segments. That old content wasn't cosmetic: since collision avoidance checks each not-yet-processed primary's *current* segments as an obstacle for whichever primary is solved earlier in a given order, stale leftover geometry could silently make the search harder (or even fail) for reasons that had nothing to do with the actual scenario being solved now.

Generate All Primaries now resets every primary to a blank default (one straight segment at Initial Primary Length -- exactly what a freshly-created Header primary starts with) before the very first search attempt, so every run is order-independent and reproducible regardless of what was on the primaries beforehand. If the Header assigned no Collector, or the search fails outright after trying everything, the user's actual pre-click layout is restored exactly as before -- the reset only ever affects the search's own internal starting point, never what's left behind on total failure.

Implementation: a new `_blank_primary_state(hs)` / `_blank_header_primary_states(header, routes)` pair in `operators.py` builds the same snapshot shape `_snapshot_primary_state` already produces, but with fresh-default segment content and no assisted-solver metadata. `_solve_header_primary_set` computes this once up front and uses it (instead of the pre-click snapshot) for the *per-attempt* restore inside `_try_all_phases`; the pre-click snapshot is kept, unchanged, purely for the final restore-on-total-failure paths. `_solve_header_primary_set_fixed_phase` needed no changes at all -- it snapshots "whatever's on the routes right now" at its own start, which is already blank by the time it's called, so the reset composes through automatically.

One notable, and good, side effect discovered while re-running the full smoke test: the pre-existing "tiny search budget" demonstration (RUN 4 in `profile_smoke_test.py`) reuses a Header that a prior run had already solved, specifically to test that the budget cutoff engages. With the reset in place, that scenario now *succeeds* instead of failing, because the stale prior solution is no longer sitting there as an artificial extra collision obstacle -- the search genuinely got easier, not broken. The demonstration script's comments were updated to explain this rather than left describing stale behavior; the budget-cutoff mechanism itself (checked only between whole attempts, never mid-attempt) is completely unchanged.

Verified with a new dedicated test (`test_generate_all_resets.py`, not shipped): `_blank_primary_state` produces the exact expected fresh-default shape; a live Blender pipeline test deliberately corrupts both primaries on a 2-primary Header with distinctive non-blank content, then intercepts the solver's very first per-primary solve call to confirm every primary already had exactly one blank STRAIGHT segment at Initial Primary Length *before* any solving began, not the corrupted content; and a separate case confirms an early failure (no Collector assigned, before the search loop is ever reached) leaves the corrupted pre-click layout completely untouched, not reset to blank. The existing standalone correctness suites, the full UI/mixed-CLR/Complete-to-Collector test suite, and the full 4-run smoke test were all re-run and remain unaffected (aside from the expected, understood, and now-documented RUN 4 outcome change above).

## v0.14 status

The keep-out / collector-phase / guide-spline assisted-routing branch remains experimental/unfinished. v0.14.5 reorganized the physical Header/Collector/Route interfaces; v0.14.6 added solver profiling; v0.14.7 fixed the primary-collision per-call bottleneck that profiling found; v0.14.8 bounds the remaining combinatorial search volume (order comparison count + an overall time budget) so a Header that can't be solved fails fast instead of hanging; v0.14.9 corrects a false non-determinism alarm from v0.14.8's own testing notes (no code change); v0.14.10 fixes the keep-out mesh rebuild redundancy and accelerates the deep-embedding fallback check; v0.14.11 speeds up the exact per-triangle distance primitive via shared precomputation; v0.14.12 adds a verified-safe early-exit contract to that primitive, and separately attempted (then reverted) a reduction to the coarse clocking-search grid after full-pipeline testing caught a regression synthetic testing had missed; v0.14.13 root-causes that regression (an emergent property of sequential greedy multi-primary solving in near-zero-slack scenarios, not a bug) and re-ships the density reduction with a dense-search fallback that recovers exactly the failure mode found; v0.14.14 extends the same fast/dense pattern to keep-out collision-search resolution; v0.14.15 extends it once more to the a/b/offset coarse grid itself -- the single biggest cost driver of the three -- with a measured, verified win on the 6-primary reference scenario (~99-107s down to 82.6s) achieved entirely through the fast path with no fallback needed; v0.14.16 fixes a budget-starvation bug in the fallback itself, found by finally running the realistic dense-mesh benchmark against the accumulated changes -- the fast and dense searches now each get their own full time budget instead of sharing one; v0.14.17 adds "Complete to Collector," letting a user hand-place the start of a primary and have the assisted solver finish only the remainder, reusing the existing solver/geometry machinery unmodified aside from a frame re-anchor and a seam-aware self-collision trim; v0.14.18 is a pure UI/UX pass: replaced the ~290-line always-fully-expanded Header inspector with 14 proper collapsible sub-panels, added plain-language tooltips to every user-facing property and operator button, and moved "Complete to Collector" from the Header's replace-everything Generate buttons to the per-primary Route panel where it belongs alongside Rebuild; v0.14.19 adds mixed-CLR support -- an optional tighter Start CLR for just the first bend nearest the flange, matching common real-world fabrication practice, fully backward compatible and with no new search cost since the start radius is a fixed input rather than a searched dimension; v0.14.20 makes Primary Count and Port Spacing apply automatically instead of requiring a separate button click; v0.14.21 makes Generate All Primaries always search from a blank per-primary slate instead of whatever leftover content happened to be on each primary, so results are reproducible and no longer silently biased by stale prior geometry acting as an extra collision obstacle. All three `dense_search` dimensions (clocking grid, collision bend-sample resolution, a/b/offset grid) are covered by the same single fallback flag and the same fast-then-dense retry in `_solve_header_primary_set`, which now correctly gives each attempt room to actually run. The candidate grid density remains something to tune carefully -- always against real reference scenarios (plural, and varied, not just the cheapest one) -- but the group-solve path now has a safety net covering all three dimensions that can go wrong this way, without that safety net silently starving itself.
