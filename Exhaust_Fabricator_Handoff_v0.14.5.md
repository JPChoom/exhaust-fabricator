...
Requested directly: "When clicking Generate All primaries reset the current layout so it can start from zero." Every phase/order attempt inside `_solve_header_primary_set` previously started from a snapshot of whatever was already on each primary the moment the button was clicked (`_restore_header_primary_states(routes, original_states)` at the top of each phase-candidate iteration in `_try_all_phases`). That mattered for more than cosmetics: `_collision_obstacles_for_primary`'s default `collision_routes=None` falls back to `header_primary_objects(header)` -- ALL primaries, including ones not yet reached in the order currently being tried -- so a not-yet-processed primary's *stale* segments (leftover from a prior Generate, a hand-built Complete-to-Collector prefix, an experimental manual edit) were a real collision obstacle for whichever primary got solved earlier in that attempt, entirely incidental to the actual scenario being searched for right now.

**The fix.** Two new functions in `operators.py`: `_blank_primary_state(hs)` builds a snapshot in the exact shape `_snapshot_primary_state` already produces (`{'segments': (data, active_index), 'meta': {}}`), but with one default STRAIGHT segment at `hs.initial_primary_length` -- the same content `_create_header_primary` gives a brand-new primary -- and empty `meta` (so `_restore_primary_state` deletes every `_SOLVER_STATE_KEYS` custom property, correctly reflecting "never generated"). `_blank_header_primary_states(header, routes)` maps that across every primary. `_solve_header_primary_set` computes `blank_states` once, right next to the existing `original_states` snapshot, and swaps the per-attempt restore inside `_try_all_phases` from `original_states` to `blank_states` -- the two remaining `original_states` restores (search fails outright; winning phase fails the final hard gate) are untouched, so a totally failed search still gives back the user's actual pre-click layout, never leaves it blank. `_solve_header_primary_set_fixed_phase` needed no code change at all: it snapshots "whatever's currently on the routes" (`original = _snapshot_header_primary_states(routes)`) at its own start, and by the time it's called the routes are already blank, so the reset composes through for free.

**A side effect worth documenting, not a bug.** Re-running the full smoke test surfaced a real change in RUN 4 (`profile_smoke_test.py`): it reuses the same 6-primary Header that RUN 3 just solved, shrinks `solver_max_solve_seconds` to the property's 5s floor, and re-runs Generate All specifically to demonstrate the budget cutoff engaging. Before this fix, that scenario reliably reported "no solution found" within the tiny budget; after it, the search actually *succeeds*, taking as long as one full fast+dense attempt pair naturally needs (~124s observed). Root cause, confirmed via the profiling breakdown (`phase_attempt`/`order_attempt`/`primary_solve` call counts): RUN 3's own complex solved geometry was previously sitting there as an extra, artificial collision obstacle during RUN 4's search, on top of the deliberately reused keep-out mesh; once every primary resets to a trivial blank stub instead, the fast search's first order attempt genuinely got easier and started succeeding where it previously failed. The budget-cutoff *mechanism* itself is completely unchanged -- still checked only between whole attempts, never mid-attempt, per its own long-documented contract ("actual run time can run a bit over this number too") -- RUN 4 just no longer happens to hit that cutoff for this particular reused-header scenario, because the search is legitimately more capable now. `profile_smoke_test.py`'s RUN 4 comments were rewritten to explain this rather than left describing behavior that no longer occurs; the script itself is a dev-only tool, not shipped, so no user-facing consequence.

**Verification** (`test_generate_all_resets.py`, not shipped): `_blank_primary_state` produces exactly one default STRAIGHT segment at `initial_primary_length` with empty solver metadata; a full Blender pipeline test deliberately corrupts both primaries on a 2-primary Header with distinctive non-blank content (a 40"+bend+12" three-segment layout, versus the 24" single-segment default) and monkeypatches `_solve_one_header_primary` to record every primary's segment count/kind/length at the moment the very first solve call happens -- confirming all primaries already show exactly 1 blank STRAIGHT segment at the default length before any solving occurs, not the corrupted content; and a separate case (no Collector assigned, so the search loop is never reached) confirms the corrupted pre-click layout is left completely untouched, not reset to blank, on that early failure path. The existing standalone correctness suites, the mixed-CLR suite, and the full UI/Complete-to-Collector test suite were all re-run and remain unaffected; the full 4-run smoke test was re-run and remains unaffected except for the RUN 4 outcome change documented above, which was investigated and understood rather than assumed benign.
